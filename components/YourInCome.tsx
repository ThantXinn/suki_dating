...
"use client";
import { yourIncome } from "@/app/constants";
...
const YourInCome = ({
  ...
  incomeCategory: string;
  updateMultiFormData: (feilds: Partial<incomeCategoryData>) => void;
  icon: ReactElement;
  isItemSelect: boolean;
  setIsItemSelect: Dispatch<SetStateAction<boolean>>;
}) => {
  useEffect(() => {
    const incomeTitles = yourIncome.map(({ title }) => title);
    //if user selected item is exist return true;
    isItemSelect = incomeTitles.includes(incomeCategory);
      ...
      <div className='relative bg-slate-50 w-full'>
        <Label htmlFor='sub-title'>
          <h1 className='text-2xl text-center font-semibold py-5'>
            What is your anual income...?
          </h1>
        </Label>
        <hr className='w-full border border-slate-500' />
      </div>
      <div className='overflow-scroll bg-slate-50 h-full flex flex-wrap items-center justify-center gap-2 py-16 px-28'>
        {yourIncome.map(({ id, title }) => (
          <div key={id}>
            <Button
              variant={"ghost"}
              ...
              onClick={() => updateMultiFormData({ incomeCategory: title })}>
